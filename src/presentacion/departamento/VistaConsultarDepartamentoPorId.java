...
public class VistaConsultarDepartamentoPorId extends JFrame implements Observador {
	public void actualizar(Object datos) {
		...
		JLabel titulo = new JLabel("Consultar departamento por ID");
		...
		aceptar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					controlador.accion(new Contexto(Evento.CONSULTAR_DEPARTAMENTO_POR_ID, id));
				} catch (NumberFormatException n) {
					controlador.accion(new Contexto(Evento.CONSULTAR_DEPARTAMENTO_POR_ID, 0));
				}
			}
		});
