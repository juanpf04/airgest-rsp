...
public class VistaConsultarModeloPorId extends JFrame implements Observador {
	...
	public void actualizar(Object datos) {
		...
		JLabel titulo = new JLabel("Consultar modelo por ID");
		...
		aceptar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					controlador.accion(new Contexto(Evento.CONSULTAR_MODELO_POR_ID, id));
				} catch (NumberFormatException n) {
					controlador.accion(new Contexto(Evento.CONSULTAR_MODELO_POR_ID, 0));
				}
			}
		});
